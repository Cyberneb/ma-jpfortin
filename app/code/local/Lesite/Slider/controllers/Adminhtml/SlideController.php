 ...
 */
class Lesite_Slider_Adminhtml_SlideController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction() {
	    $slider_id = $this->getRequest()->getParam('slider_id');
            if (!$slider_id) {
               Mage::getSingleton('adminhtml/session')->addError(Mage::helper('slider')->__('Slide id not set'));
               $this->_redirect('slider/adminhtml_slider/index');         
            } 
            else
            {
                ...
                if (!$slider->hasData()) {
                    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('slider')->__('Slide with id=%s not exists', $slider_id));
                    $this->_redirect('slider/adminhtml_slider/index');  
                }
                else
                {
	...
	public function editAction() {
    		...
    		if ($model->getId() || $id == 0) {
    			...
    			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('slider')->__('Slide does not exists'));
	...
	public function saveAction()
	    ...
	    $slider_id = $this->getRequest()->getParam('slider_id');	   
 	    if ($data = $this->getRequest()->getPost())
	        ...
	        try 
                {
                    $upload_dir  = Mage::getBaseDir('media').DS.'slider'.DS;
                    if (!file_exists($upload_dir)) mkdir($upload_dir, 0755, true);
                    
                    if ( isset( $_FILES['image_file']['name']) && ( file_exists($_FILES['image_file']['tmp_name']) ) )
                    {
                        $uploader = new Varien_File_Uploader('image_file');
                        $uploader->setAllowedExtensions( array('jpg','jpeg','gif','png') );
                        $uploader->setAllowRenameFiles(true);
                        $uploader->setFilesDispersion(false);
                        $uploader->save($upload_dir, $_FILES['image_file']['name']);
                        $data['image_path'] = $uploader->getUploadedFileName();
                    }
                    elseif( empty( $data['image_path'] ) )
                    {              
                        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('slider')->__("File can't be uploaded. Slider_id not set."));
		    ...
		    if ($this->getRequest()->getParam('back'))
                    {
			$this->_redirect('*/*/edit', array('id' => $model->getId(), 'slider_id' => $model->getData('slider_id')));
		        return;
		    }
		    $this->_redirect('*/*/', array('slider_id' => $model->getData('slider_id')));
		    return;
                } catch (Exception $e) {
                    Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                    Mage::getSingleton('adminhtml/session')->setFormData($data);
                    $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id'),'slider_id' => $model->getData('slider_id')));
                    return;
                }
            }
 	...
 	public function deleteAction() {
		...
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('slider')->__('Slide was successfully deleted'));
				$this->_redirect('*/*/', array('slider_id' => $slider_id));
			} catch (Exception $e) {
				Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
        ...
        public function uploadAction()
	{
	       $slider_id = $this->getRequest()->getParam('slider_id');
            ...
            if ($slider_id) {
                $slider_dimensions = Mage::getModel('slider/slider')->load($slider_id)->toArray(array('width','height'));
                
                $upload_dir  = Mage::getBaseDir('media').DS.'slider'.DS;
    
               if (!file_exists($upload_dir)) mkdir($upload_dir, 0755, true);
                
                $uploader = Mage::getModel('slider/fileuploader');
                ...
                $result = array('error' => Mage::helper('slider')->__("File can't be uploaded. Slider_id not set.")); 
            }
    ...
    public function massDeleteAction() {
        ...
        if(!is_array($slideIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('slider')->__('Please select slide(s)'));
        } else {
            try {
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('slider/adminhtml_slide_grid')
            ->getCsv();
