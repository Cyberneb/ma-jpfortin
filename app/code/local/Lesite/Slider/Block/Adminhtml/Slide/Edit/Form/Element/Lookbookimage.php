 ...
 */
class Lesite_Slider_Block_Adminhtml_Slide_Edit_Form_Element_Lookbookimage extends Varien_Data_Form_Element_Abstract
{
 ...
 {
    $block_class =  Mage::getBlockSingleton('slider/adminhtml_slide_edit_form');
    $slider_id = Mage::registry('slide_data')->getData('slider_id'); 
    $upload_action  = Mage::getUrl('slider/adminhtml_slide/upload', array('slider_id'=>$slider_id, '_secure'=>true)).'?isAjax=true';
    ...
    $html = '<script type="text/javascript">
                    ...
                    img_uploader = new qq.FileUploader({
                        ...
                        onComplete: function(id, fileName, responseJSON){                           
                                    if (responseJSON.success) 
                                    {
                                       ...
                                       $html .= ' src="'.$media_url.'slider/\'+responseJSON.filename+\'" alt="\'+responseJSON.filename+\'"'; 
                                       $html .= ' width="\'+responseJSON.dimensions.width+\'" height="\'+responseJSON.dimensions.height+\'"/>\');
                                        ...
                                        jQuery(\'#image_path\').val(\'slider/\'+responseJSON.filename);
        ...
        if ($this->getValue()) {
            ...
            $img_path = $upload_folder_path.$this->getValue();
                ...
                $dimensions = Mage::helper('slider')->getImageDimensions($img_path);
